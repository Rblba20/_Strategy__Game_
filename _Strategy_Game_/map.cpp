...
Map::Map(const ld::MapDefinition &map_definition,
    ...
    for (unsigned row = 0; row < ld::config::ROWS; row++) {
      ...
      try {
          ...
          tiles.push_back(ld::Tile(texture, tile_config.get_type(), row, col));
      } catch (const std::out_of_range &) {
          std::cout << "Filename " << tile_config.get_filename() << " not found"
                    << std::endl;
      }
    }
  }
...
void Map::clean_up_units() {
  ...
  units.erase(std::remove_if(
                  ...
                  [&](const auto &unit) { return unit->get_strength() <= 0; }),
              units.end());
}
...
void Map::add_game_resource() {
  ...
  while (true) {
    ...
    if (tile.get_type() != ld::TileType::Water and !tile.game_resource_) {
      auto game_resource =
          ld::GameResource::build(*resources, game_resource_type, filename);
